...
router.delete("/experience/:exp_id", auth, async (req, res) => {
  try {
    ...
    if (removeIndex === -1) {
      return res.status(400).json({ msg: "Bad param exp_id" });
    }

    // splice removes the experience
...
router.delete("/education/:edu_id", auth, async (req, res) => {
  try {
    ...
    if (removeIndex === -1) {
      return res.status(400).json({ msg: "Bad param edu_id" });
    }

    // splice removes the education
  ...
  try {
    const options = {
      uri: `https://api.github.com/users/${
        req.params.username
      }/repos?per_page=5&sort=created:asc&client_id=${getEnvConfig(
        "githubClientId"
      )}&client_secret=${getEnvConfig("githubSecret")}`,
      method: "GET",
